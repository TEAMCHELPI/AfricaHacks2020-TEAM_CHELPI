<template>
  ...
  <section class="w-full">
    <form @submit.prevent="completeOrder" class="flex flex-col w-full">
      <label for="schedule-now" :class="['p-4 flex flex-grow w-full border-l-4 border-transparent', {'bg-gray-400 bg-opacity-25 border-brand-primary':scheduleType=='now'}]">
       ...
       <hr>
      <label for="schedule-later" :class="['p-4 flex flex-grow w-full border-l-4 border-transparent', {'bg-gray-400 bg-opacity-25 border-brand-primary':scheduleType=='later'}]">
